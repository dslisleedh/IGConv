...
class MSRModel(BaseModel):
    def __init__(self, opt):
        super(MSRModel, self).__init__(opt)
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output = self.net_g_ema(self.lq, self.cur_scale)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output = self.net_g(self.lq, self.cur_scale)
    ...
    def test_discrete(self):
        ...
        with torch.no_grad():
            self.output = self.net_g_discrete(self.lq, self.cur_scale)
    ...
    def test_selfensemble(self):
        ...
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                out_list = [self.net_g_ema(aug, self.cur_scale) for aug in lq_list]
        else:
            self.net_g.eval()
            with torch.no_grad():
                out_list = [self.net_g(aug, self.cur_scale) for aug in lq_list]
            self.net_g.train()
    ...
    def test_selfensemble_discrete(self):
        def _transform(v, op):
            # if self.precision != 'single': v = v.float()
            v2np = v.data.cpu().numpy()
            if op == 'v':
        ...
        self.net_g_discrete.eval()
        with torch.no_grad():
            out_list = [self.net_g_discrete(aug, self.cur_scale) for aug in lq_list]
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if discretize_net_g and not self.opt['val'].get('self_ensemble', False):
                self.test_discrete()
            elif discretize_net_g and self.opt['val'].get('self_ensemble', False):
                self.test_selfensemble_discrete()
            elif self.opt['val'].get('self_ensemble', False):
