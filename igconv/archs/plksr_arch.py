...
from igconv.archs.arch_utils import IGConv, IGSample, test_direct_metrics
...
class RectSparsePLKConv2d(nn.Module):
    ...
    def forward(self, x):  # No reparametrization since this is for a ablative study
        if self.training:
...
class PLKSRIGPlus(nn.Module):
    def __init__(
        self, dim: int, n_blocks: int,
        # CCM options
        ccm_type: Literal['CCM', 'ICCM', 'DCCM'],
        # LK Options
        ...
        self.to_rgb = IGConv(
            dim, sk_kernel_size, implicit_dim, latent_layers, geo_ensemble
        )
        self.skip = IGSample(
            dim, sk_kernel_size, implicit_dim//2, latent_layers//2, geo_ensemble
        )
        
    def forward(self, x: torch.Tensor, scale: int) -> torch.Tensor:
        # x = self.to_rgb(self.feats(x), scale, repeat_interleave(x, scale ** 2))
        feat = self.feats(x)
        x = self.to_rgb(feat, scale) + self.skip(x, feat, scale)
        return x
...
if __name__== '__main__':
    ...
    # x = torch.FloatTensor(batch_size, 3, height // upsampling_factor, width // upsampling_factor).uniform_(0, 1).type(torch.float32).clamp(0., 1.)
    shape = (batch_size, 3, height // upsampling_factor, width // upsampling_factor)
    model = PLKSRIGPlus(**model_kwargs)
    # model = PLKSRLarge(**model_kwargs)
    ...
    test_direct_metrics(model, shape, scale=upsampling_factor)
    # with torch.no_grad():
    #     x = x.cuda()
    #     model = model.cuda()
