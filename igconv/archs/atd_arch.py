...
Github: 'https://github.com/LabShuHangGU/Adaptive-Token-Dictionary/blob/main/basicsr/archs/arch_util.py'
'''
...
from igconv.archs.arch_utils import IGConv, IGSample
...
class ATD_CA(nn.Module):
    ...
    def forward(self, x, td, x_size):
        ...
        # Q @ K^T
        attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1).contiguous())  # b, n, n_tk
...
class AC_MSA(nn.Module):
    ...
    def forward(self, qkv, sim, x_size):
        ...
        qkv = y.reshape(b, ng, gs, 3, self.num_heads, c//self.num_heads).permute(3, 0, 1, 4, 2, 5).contiguous()  # 3, b, ng, nh, gs, c//nh
        q, k, v = qkv[0], qkv[1], qkv[2]

        # Q @ K^T
        attn = (q @ k.transpose(-2, -1).contiguous())  # b, ng, nh, gs, gs
        ...
        # Attn * V
        y = (attn @ v).permute(0, 1, 3, 2, 4).reshape(b, n+pad_n, c).contiguous()[:, :n, :]
...
class ATDTransformerLayer(nn.Module):
    ...
    def forward(self, x, td, x_size, params):
        ...
        if not self.is_last:
            mask_soft = self.softmax(self.norm3(sim_atd.transpose(-1, -2).contiguous()))
...
class ATD(nn.Module):
    ...
    def __init__(self,
        ...
        elif self.upsampler == 'igconv':
            self.upsample = IGConv(
                embed_dim, sk_kernel_size, implicit_dim, latent_layers, geo_ensemble
            )
        elif self.upsampler == 'igconv+igsample':
            self.upsample = IGConv(
                embed_dim, sk_kernel_size, implicit_dim, latent_layers, geo_ensemble, max_s
            )
            self.skip = IGSample(
                embed_dim, sk_kernel_size, implicit_dim//2, latent_layers//2, geo_ensemble, max_s
            )
        else:
    ...
    def forward(self, x, scale=None):
        ...
        if self.upsampler == 'igconv+igsample':
            inp = x
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, params)) + x
            x = self.upsample(x, scale)
        elif self.upsampler == 'igconv+igsample':
            x = self.conv_first(x)
            x_feat = self.conv_after_body(self.forward_features(x, params)) + x
            x = self.upsample(x_feat, scale)
        ...
        # return x
        if self.upsampler == 'igconv+igsample':
            skip = self.skip(inp, x_feat, scale)[:, :, :h_ori * scale, :w_ori * scale]
