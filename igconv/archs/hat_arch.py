...
modifications were made to prevent gradient bucket error related with transpose or permute
"""
import math
import torch
...
class OCAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi):
        ...
        attn = self.softmax(attn)
        attn_windows = (attn @ v).transpose(1, 2).reshape(b_, nq, self.dim).contiguous()
...
class HAT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.skip = IGSample(
                dim=embed_dim, kernel_size=sk_kernel_size, implicit_dim=implicit_dim//2,
                latent_layers=latent_layers//2, geo_ensemble=geo_ensemble
            )
        else:
    ...
    def forward(self, x, scale=None):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_first(x)
            x_feat = self.conv_after_body(self.forward_features(x)) + x
            x = self.upsample(x_feat, scale)
            
        x = x / self.img_range + self.mean

        if self.upsampler == 'igconv+igsample':
            skip = self.skip(inp, x_feat, scale)[:, :, :H*scale, :W*scale]
            x = x[:, :, :H*scale, :W*scale] + skip
