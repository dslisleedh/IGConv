"""
Coeds from SMFANet
https://github.com/Zheng-MJ/SMFANet/blob/main/basicsr/archs/SMFANet_arch.py
"""
import torch
...
@ARCH_REGISTRY.register()
class SMFANetIG(nn.Module):
    def __init__(
        self, dim=36, n_blocks=8, ffn_scale=2, upscaling_factor=4,
        # IGConv
    ...
    def forward(self, x, scale=None):
        x = self.to_feat(x)
        x = self.feats(x) + x
        x = self.to_img(x, scale)
        return x


@ARCH_REGISTRY.register()
class SMFANetIGPlus(nn.Module):
    def __init__(
        self, dim=36, n_blocks=8, ffn_scale=2, upscaling_factor=4,
        # IGConv
        sk_kernel_size=3, implicit_dim=256,
        latent_layers=4, geo_ensemble=True,
        # DySample
    ...
    def forward(self, x, scale=None):
        ...
        x = self.to_feat(x)
        feat = self.feats(x) + x
        x = self.to_img(feat, scale)
        skip = self.skip(inp, feat, scale)
        x = x + skip
