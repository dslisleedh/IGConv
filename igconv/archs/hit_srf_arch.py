"""
Coeds from HiT-SR
https://github.com/XiangZ-0/HiT-SR/blob/main/basicsr/archs/hit_srf_arch.py
"""
import math
import torch
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.upsample = IGConv(
                embed_dim, sk_kernel_size, implicit_dim, latent_layers, False
            )
        elif self.upsampler == 'igconv+igsample':
            self.upsample = IGConv(
                embed_dim, sk_kernel_size, implicit_dim, latent_layers, True
            )
            self.return_skip = return_skip
            self.skip = IGSample(
                embed_dim, sk_kernel_size, implicit_dim//2, latent_layers//2, True
            )
        else:
